...
-   names: lo usamos cuando queremos conocer los nombres de las variables que componen la base de datos. Solo necesita un argumento básico: el nombre de la data.
...
-   sum: La suma de todos los valores dentro de un conjunto de datos
...
Ahora tu, practica cambiando el nombre de la variable P42_1 a porcen_cobertura
...
Ahora tu, practica seleccionando las variables Tipomuni y P44_1. Recuerda que cada una debe ir separada de una coma (,) 
...
data %>% 
  filter(Departamento == "Loreto"&Tipomuni == 2) %>% 
    summarise(maximo = which.max(recojo_diario_kg))
...
data %>% 
  ...
  summarise(minimo=min(recojo_diario_kg))
```
